...
export default function ArtHombre() {
  ...
  const initialCamisasCategories = [
    { id: 'todos', name: 'Todas las Camisas', count: 0 },
    { id: 'formales', name: 'Camisas Formales', count: 0 },
    { id: 'casuales', name: 'Casuales', count: 0 },
    { id: 'deportivas', name: 'Deportivas', count: 0 },
    { id: 'verano', name: 'Verano', count: 0 },
      ...
      if (isCurrentlyFavorite) {
        ...
        if (result.status === 'success') {
          ...
          Swal.fire({
            ...
            text: 'La camisa se ha removido de tus favoritos',
            icon: 'success',
            timer: 1500,
        ...
        if (result.status === 'success') {
          ...
          Swal.fire({
            ...
            text: 'La camisa se ha agregado a tus favoritos',
            icon: 'success',
            timer: 1500,
            showConfirmButton: false
          });
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== 'todos') {
      filtered = filtered.filter(product => 
        mapProductToCamisaCategory(product) === selectedCategory
      );
    }
  ...
  const loadDataFromAPI = async () => {
    try {
      ...
      if (productsResponse.status === 'success') {
        // Filtrar solo productos para hombre que sean camisas
        const camisasHombre = productsResponse.data.filter(product => 
          product.genero && 
          product.genero.toLowerCase() === 'hombre' &&
          // Asumimos que las camisas tienen categorías relacionadas con camisas
          (product.categoria_nombre?.toLowerCase().includes('camis') ||
           product.nombre?.toLowerCase().includes('camis') ||
           product.descripcion?.toLowerCase().includes('camis'))
        );

        // Si no hay camisas en la API, usar datos mock
        if (camisasHombre.length === 0) {
          loadMockData();
        } else {
          setProducts(camisasHombre);
          setFilteredProducts(camisasHombre);
          updateCategoriesCount(camisasHombre);
        }
      } else {
      ...
      Swal.fire({
        title: 'Error',
        text: 'No se pudieron cargar las camisas. Mostrando datos de ejemplo.',
        icon: 'error',
        confirmButtonText: 'Entendido'
      });
      loadMockData();
  ...
  // Función para mapear productos a categorías de camisas
  const mapProductToCamisaCategory = (product) => {
    ...
    if (categoria.includes('camiseta') || categoria.includes('basica') || categoria.includes('básica')) {
      return 'camisetas';
    }
    if (categoria.includes('formal') || categoria.includes('ejecutiva') || categoria.includes('vestir')) {
      return 'formales';
    }
    if (categoria.includes('casual') || categoria.includes('diario') || categoria.includes('informal')) {
      return 'casuales';
    }
    if (categoria.includes('deportiv') || categoria.includes('deporte') || categoria.includes('sport')) {
      return 'deportivas';
    }
    if (categoria.includes('verano') || categoria.includes('lino') || categoria.includes('fresca')) {
      return 'verano';
    }
    if (categoria.includes('denim') || categoria.includes('jean') || categoria.includes('mezclilla')) {
      return 'denim';
    }

    // LUEGO buscar en nombre y descripción
    if (nombre.includes('básica') || nombre.includes('basica') || descripcion.includes('básica')) {
      return 'camisetas';
    } else if (nombre.includes('formal') || descripcion.includes('formal') || descripcion.includes('negocios') || descripcion.includes('ejecutiv')) {
      return 'formales';
    } else if (nombre.includes('casual') || descripcion.includes('casual') || descripcion.includes('diario') || descripcion.includes('informal')) {
      return 'casuales';
    } else if (nombre.includes('deportiv') || descripcion.includes('deportiv') || descripcion.includes('deporte') || descripcion.includes('sport')) {
      return 'deportivas';
    } else if (nombre.includes('verano') || descripcion.includes('verano') || descripcion.includes('lino') || descripcion.includes('fresca')) {
      return 'verano';
    } else if (nombre.includes('denim') || descripcion.includes('denim') || descripcion.includes('jean') || descripcion.includes('mezclilla')) {
      return 'denim';
    }
    
    // Si la categoría es simplemente "Camisas", asignar a "casuales" por defecto
  ...
  const loadMockData = () => {
    const mockProducts = [
      {
        id: 1,
        nombre: "Camisa Casual Elegante",
        descripcion: "Camisa de algodón premium para look casual",
        precio: 45.99,
        imagen_url: "https://via.placeholder.com/300x300/3B82F6/FFFFFF?text=Camisa+Casual",
        stock: 10,
        genero: "hombre",
        categoria_nombre: "camisas casuales",
        rating: 4.5,
      ...
      {
        id: 2,
        nombre: "Camisa Formal Ejecutiva",
        descripcion: "Camisa formal para reuniones de negocios",
        precio: 59.99,
        imagen_url: "https://via.placeholder.com/300x300/374151/FFFFFF?text=Camisa+Formal",
        stock: 5,
        genero: "hombre",
        categoria_nombre: "camisas formales",
        rating: 4.8,
        creado_en: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
        tallas: ['M', 'L', 'XL'],
        colores: ['Blanco', 'Azul claro', 'Rosa palo']
      }
    ];
  ...
  const updateCategoriesCount = (productsList) => {
    const updatedCategories = initialCamisasCategories.map(category => {
      if (category.id === 'todos') {
        return { ...category, count: productsList.length };
      }
      
      const count = productsList.filter(product => 
        mapProductToCamisaCategory(product) === category.id
      ).length;
        ...
        <div className="container">
          <div className="art-section-title">
            ...
            <p className="art-subtitle">
              Descubre nuestra exclusiva colección de camisas masculinas para cada ocasión
            </p>
          </div>
          ...
          <div className="art-main-layout">
            {/* SIDEBAR DE CATEGORÍAS DE CAMISAS */}
            <aside className="art-categories-sidebar">
              <h3 className="art-categories-title">Tipos de Camisas</h3>
            ...
            <main className="art-products-section">
              <div className="art-products-header">
                <div className="art-products-count">
                  {filteredProducts.length} camisa{filteredProducts.length !== 1 ? 's' : ''} encontrada{filteredProducts.length !== 1 ? 's' : ''}
              ...
              <div className="art-product-grid">
                {filteredProducts.length === 0 ? (
                  <div className="art-no-products">
                    ...
                    <h3>No se encontraron camisas</h3>
                      ...
                      <div key={product.id} className="art-product-card">
                        ...
                        <div className="art-product-info">
                          ...
                          <div className="art-product-category">
                            {categories.find(cat => cat.id === mapProductToCamisaCategory(product))?.name}
                          </div>
