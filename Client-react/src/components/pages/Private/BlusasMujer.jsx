...
export default function BlusasMujer() {
  ...
  // Categorías específicas para blusas - como estado
  const initialBlusasCategories = [
    { id: 'todos', name: 'Todas las Blusas', count: 0 },
    { id: 'formales', name: 'Blusas Formales', count: 0 },
    { id: 'casuales', name: 'Casuales', count: 0 },
    { id: 'elegantes', name: 'Blusas Elegantes', count: 0 },
    { id: 'verano', name: 'Blusas de Verano', count: 0 },
    { id: 'encaje', name: 'Blusas de Encaje', count: 0 },
    { id: 'seda', name: 'Blusas de Seda', count: 0 },
    { id: 'mangas', name: 'Blusas de Mangas', count: 0 }
  ];
  ...
  useEffect(() => {
    ...
    setCategories(initialBlusasCategories);
    loadDataFromAPI();
      ...
      if (isCurrentlyFavorite) {
        ...
        if (result.status === 'success') {
          ...
          Swal.fire({
            ...
            text: 'La blusa se ha removido de tus favoritos',
            icon: 'success',
            timer: 1500,
        ...
        if (result.status === 'success') {
          ...
          Swal.fire({
            ...
            text: 'La blusa se ha agregado a tus favoritos',
            icon: 'success',
            timer: 1500,
            showConfirmButton: false
          });
  ...
  useEffect(() => {
    ...
    // Filtrar por categoría de blusa
    if (selectedCategory !== 'todos') {
      filtered = filtered.filter(product => 
        mapProductToBlusaCategory(product) === selectedCategory
      );
    }
  ...
  const loadDataFromAPI = async () => {
    try {
      ...
      if (productsResponse.status === 'success') {
        // Filtrar solo productos para mujer que sean blusas
        const blusasMujer = productsResponse.data.filter(product => 
          product.genero && 
          product.genero.toLowerCase() === 'mujer' &&
          // Asumimos que las blusas tienen categorías relacionadas con blusas
          (product.categoria_nombre?.toLowerCase().includes('blus') ||
           product.nombre?.toLowerCase().includes('blus') ||
           product.descripcion?.toLowerCase().includes('blus'))
        );

        // Si no hay blusas en la API, usar datos mock
        if (blusasMujer.length === 0) {
          loadMockData();
        } else {
          setProducts(blusasMujer);
          setFilteredProducts(blusasMujer);
          updateCategoriesCount(blusasMujer);
        }
      } else {
      ...
      Swal.fire({
        title: 'Error',
        text: 'No se pudieron cargar las blusas. Mostrando datos de ejemplo.',
        icon: 'error',
        confirmButtonText: 'Entendido'
      });
      loadMockData();
  ...
  // Función para mapear productos a categorías de blusas
  const mapProductToBlusaCategory = (product) => {
    ...
    if (categoria.includes('formal') || categoria.includes('ejecutiva') || categoria.includes('oficina')) {
      return 'formales';
    }
    if (categoria.includes('casual') || categoria.includes('diario') || categoria.includes('informal')) {
      return 'casuales';
    }
    if (categoria.includes('elegante') || categoria.includes('noche') || categoria.includes('fiesta')) {
      return 'elegantes';
    }
    if (categoria.includes('verano') || categoria.includes('lino') || categoria.includes('fresca') || categoria.includes('sandalias')) {
      return 'verano';
    }
    if (categoria.includes('encaje') || categoria.includes('lace') || categoria.includes('transparente')) {
      return 'encaje';
    }
    if (categoria.includes('seda') || categoria.includes('silk') || categoria.includes('satén')) {
      return 'seda';
    }
    if (categoria.includes('manga') || categoria.includes('sleeve') || categoria.includes('larga') || categoria.includes('corta')) {
      return 'mangas';
    }

    // LUEGO buscar en nombre y descripción
    if (nombre.includes('formal') || descripcion.includes('formal') || descripcion.includes('oficina') || descripcion.includes('ejecutiv')) {
      return 'formales';
    } else if (nombre.includes('casual') || descripcion.includes('casual') || descripcion.includes('diario') || descripcion.includes('informal')) {
      return 'casuales';
    } else if (nombre.includes('elegante') || descripcion.includes('elegante') || descripcion.includes('fiesta') || descripcion.includes('noche')) {
      return 'elegantes';
    } else if (nombre.includes('verano') || descripcion.includes('verano') || descripcion.includes('lino') || descripcion.includes('fresca')) {
      return 'verano';
    } else if (nombre.includes('encaje') || descripcion.includes('encaje') || descripcion.includes('lace')) {
      return 'encaje';
    } else if (nombre.includes('seda') || descripcion.includes('seda') || descripcion.includes('silk')) {
      return 'seda';
    } else if (nombre.includes('manga') || descripcion.includes('manga')) {
      return 'mangas';
    }
    
    // Si la categoría es simplemente "Blusas", asignar a "casuales" por defecto
    if (categoria.includes('blusa')) {
      return 'casuales';
    }
    ...
    const mockProducts = [
      {
        ...
        descripcion: "Blusa de seda natural con corte elegante para ocasiones especiales",
        precio: 65.99,
        imagen_url: "https://via.placeholder.com/300x300/EC4899/FFFFFF?text=Blusa+Elegante",
        stock: 8,
        genero: "mujer",
        categoria_nombre: "blusas elegantes",
        rating: 4.7,
        creado_en: new Date().toISOString(),
        tallas: ['XS', 'S', 'M', 'L'],
        colores: ['Blanco', 'Negro', 'Rosa', 'Azul claro']
      },
      {
        id: 2,
        nombre: "Blusa Casual de Algodón",
        descripcion: "Blusa cómoda de algodón para uso diario, perfecta para looks casuales",
        precio: 35.99,
        imagen_url: "https://via.placeholder.com/300x300/8B5CF6/FFFFFF?text=Blusa+Casual",
        stock: 12,
        genero: "mujer",
        categoria_nombre: "blusas casuales",
        rating: 4.3,
        creado_en: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
        tallas: ['S', 'M', 'L', 'XL'],
        colores: ['Blanco', 'Beige', 'Verde menta', 'Azul']
      },
      {
        id: 3,
        nombre: "Blusa Formal para Oficina",
        descripcion: "Blusa formal con detalles sofisticados ideal para el entorno laboral",
        precio: 49.99,
        imagen_url: "https://via.placeholder.com/300x300/374151/FFFFFF?text=Blusa+Formal",
        stock: 6,
        genero: "mujer",
        categoria_nombre: "blusas formales",
        rating: 4.6,
        creado_en: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
        tallas: ['XS', 'S', 'M'],
        colores: ['Blanco', 'Azul marino', 'Gris', 'Negro']
      },
      {
        id: 4,
        nombre: "Blusa de Encaje Veraniega",
        descripcion: "Blusa ligera de encaje perfecta para los días cálidos de verano",
        precio: 42.99,
        imagen_url: "https://via.placeholder.com/300x300/F59E0B/FFFFFF?text=Blusa+Verano",
        stock: 10,
        genero: "mujer",
        categoria_nombre: "blusas de verano",
        rating: 4.4,
        creado_en: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
        tallas: ['S', 'M', 'L'],
        colores: ['Blanco', 'Crudo', 'Azul cielo', 'Rosa palo']
      }
    ];
  ...
  const updateCategoriesCount = (productsList) => {
    const updatedCategories = initialBlusasCategories.map(category => {
      if (category.id === 'todos') {
        return { ...category, count: productsList.length };
      }
      
      const count = productsList.filter(product => 
        mapProductToBlusaCategory(product) === category.id
      ).length;
               ...
               <div style="background: white; padding: 30px; position: relative;">
                 ...
                 <div style="margin-bottom: 30px;">
                   ...
                   <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                     ${(product.colores || ['Blanco', 'Negro', 'Rosa']).map((color, index) => `
                       <button 
                         type="button"
                 ...
                 <div style="background: linear-gradient(135deg, #fef7ed, #fffbeb); border: 1px solid #fed7aa; border-radius: 16px; padding: 20px; margin-top: 25px;">
                   <div style="display: flex; align-items: center; gap: 15px;">
                     ...
                     <div>
                       ...
                       <div style="color: #b45309; font-size: 14px;">Esta blusa incluye 1 año de garantía y soporte premium</div>
                     </div>
                   </div>
                 </div>
               </div>
      ...
      <section className="art-featured-products">
        <div className="container">
          <div className="art-section-title">
            <h2>Blusas para Mujer</h2>
            <p className="art-subtitle">
              Descubre nuestra exclusiva colección de blusas femeninas para cada ocasión
            </p>
          </div>
          ...
          <div className="art-search-container">
            <form onSubmit={handleSearch} className="art-search-box">
              <input
                ...
                placeholder="Buscar blusas por nombre, descripción..."
                value={searchTerm}
          ...
          <div className="art-main-layout">
            {/* SIDEBAR DE CATEGORÍAS DE BLUSAS */}
            <aside className="art-categories-sidebar">
              <h3 className="art-categories-title">Tipos de Blusas</h3>
            ...
            <main className="art-products-section">
              <div className="art-products-header">
                <div className="art-products-count">
                  {filteredProducts.length} blusa{filteredProducts.length !== 1 ? 's' : ''} encontrada{filteredProducts.length !== 1 ? 's' : ''}
              ...
              <div className="art-product-grid">
                {filteredProducts.length === 0 ? (
                  <div className="art-no-products">
                    ...
                    <h3>No se encontraron blusas</h3>
                      ...
                      <div key={product.id} className="art-product-card">
                        ...
                        <div className="art-product-info">
                          ...
                          <div className="art-product-category">
                            {categories.find(cat => cat.id === mapProductToBlusaCategory(product))?.name}
                          </div>
